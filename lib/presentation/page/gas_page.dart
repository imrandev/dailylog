...
class GasPage extends StatelessWidget {
  ...
  final _gasLogTextController = TextEditingController();

  final _datePickerTextController = TextEditingController();

  final _params = <String, dynamic>{};

  final _controller = Get.find<GasController>();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          AppLocalization.cylinderGas.tr,
        ),
      ),
      body: SizedBox(
                            ...
                            lineBarsData: [
                              LineChartBarData(
                                spots: _controller.chartData.map(
                                        (element) => FlSpot(element['month'], element['average_cost'])
                                ).toList(),
                                isCurved: true,
                                barWidth: 2,
                              ),
                ...
                itemBuilder: (context, index) => ListTile(
                  ...
                  trailing: SizedBox(
                    ...
                    child: Row(
                      ...
                      children: [
                        Obx(() => Text("${_controller.logList[index].price.value ?? "00.00"} Tk")),
                        Expanded(
                          child: IconButton(
                            onPressed: () {
  ...
  void _openOptionMenu(BuildContext context, GasModel model) {
    Get.bottomSheet(
      Container(
        height: 160,
        ...
        child: Column(
          children: [
            ListTile(
              onTap: () {
                _openGasLogForm(context, model: model);
              },
